...
# Fit a model for yield as the response and year as predictor 
...
ggplot(aes(x = year, y = yield), data = winne_yields) +
  geom_point(shape = 1) +
  theme_few() +
  labs(x = "Year", y = "Corn Yields", title = "Winneshiek County Corn Yield") +
  geom_smooth(method = lm, se=FALSE, size = 0.5, color = "black")


```

With a p-value of 1.77e-13 and less than $\alpha$ = 0.05, we reject the null hypothesis $\H_0 = \beta_1$. We have enough evidence to suggest there is a significant time trend. There estimated 2.457 unit increase in corn yield with each increasing year. 
...
#Plot yield(y) by year^2 (x), use fitted values from the model for quadratic line 

winne_yields$fitted <- quad_timeLM$fitted.values

ggplot(winne_yields) +
  geom_point(mapping = aes(x = yearsq, y = yield), shape = 1) +
  geom_line(mapping = aes(x = yearsq, y = fitted)) +
  theme_few() +
  labs(x = expression("Year" ^ 2), y = "Corn Yields",title = "Winneshiek County Corn Yield")
...
#Fit a model for yield and average summer maximum temperature alone 

winne_tempLM <- lm(yield~meantmax, data=winne_summer_yield)
summary(winne_tempLM)

#Plot corn yield (y) and average summer Tmax (x)

ggplot(aes(x = meantmax, y = yield), data = winne_summer_yield) +
  geom_point(shape = 1) +
  theme_few() +
  labs(x = "Summer Maximum Temperature", y = "Corn Yields", title = "Winneshiek County Corn Yield in Summer") +
  geom_smooth(method = lm, se=FALSE, size = 0.5, color = "black")

#Fit a model for yield with covariates: average summer max temp and year \
...
```{r}
#Convert county ID code into factor 
countyID <- as.factor() 

panelLM <- (~ Tmax,Tmax^2,countyID)
